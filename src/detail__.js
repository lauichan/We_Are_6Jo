...
function sendReview(e) {
  ...
  if (window.localStorage.getItem(id) !== null) {
    reviewStorage = JSON.parse(window.localStorage.getItem(id));
  }
  ...
  userReviewInfo(id);
  console.log(userReviewInfo(id));
  // 사용자 리뷰 내용 저장
  window.localStorage.setItem(id, JSON.stringify(reviewStorage));
...
function loadReview() {
  ...
  const reviewStorage = JSON.parse(window.localStorage.getItem(id)) || [];
  // console.log(ReviewList);
  Object.values(reviewStorage).forEach((userReview) => {
    const entryHtml = `
    <li id=>
